...
class Similarity():
    ...
    def Find(self, value):
        ...
        ind = -1
        power_ratio = 0.
        mean_v = sum(value) / len(value)
        normalized_val = [float(k) / max(value) for k in value]
        for i in range(len(self.Values)):
            mean_t = sum(self.Values[i]) / len(self.Values[i])
            Val = [float(k) / max(self.Values[i]) for k in self.Values[i]]
            coe = self.Coefficient(normalized_val, Val)
            if(coe > mx):
                mx = coe
                ind = i
                power_ratio = mean_v / mean_t
        if power_ratio > 1:
            power_ratio = 1.0
        return ind, mx, power_ratio

    def Coefficient(self, val1, val2):
        ave1 = sum(val1) / len(val1)
        ave2 = sum(val2) / len(val2)
        c = 0.0
        m1 = 0.0
        m2 = 0.0
